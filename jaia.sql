...
-- -- in the following query, we do group by S.age -> create groups based on S.age, i.e, for each unique age in the
-- -- age column, output only a single row, the corresponding values of all columns have to be aggregated or ommitted or
-- -- further grouped by;
-- select
-- 	S.age
-- from
...
-- -- i.e, for each unique age, rating pair in the table, output only a single row, the corresponding values of all columns
-- -- have to be aggregated or ommitted or further grouped by
select
	S.age,
    S.rating
from
...
-- -- think of having as defining a condition that will be applied after the group by and the aggregation defined by the having clause
...
-- -- age column, output only a single row, the corresponding values of all columns have to be aggregated or ommitted or
-- -- further grouped by;
-- -- next we filter the groups created
